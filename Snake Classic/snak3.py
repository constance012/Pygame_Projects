...
def main_menu():
	...
	music_on, fullscreen, res_w, res_h = frwk.read_file(
		'data/user_config.txt', music_on, fullscreen, res_w, res_h)
	...
	while True:
		...
		# Tiêu đề:
		frwk.draw_text('SNAK3: CLASSIC', 'consolas', 50, True,
					   BLACK, screen, ws[0] / 2, ws[1] / 6)

		# Nút bấm:
		button_1 = pygame.Rect(300, 240, 270, 50)
		frwk.draw_rect(button_1, screen, GREY, ws[0] / 2, ws[1] / 5 * 2)
		frwk.draw_text('Casual Mode', 'consolas', 30, False,
					   BLACK, screen, ws[0] / 2, ws[1] / 12 * 5)
		if button_1.collidepoint((mx, my)):  # Khi dí trỏ chuột vào
			frwk.draw_rect(button_1, screen, MEDIUM_GREY,
						   ws[0] / 2, ws[1] / 5 * 2)
			frwk.draw_text('Casual Mode', 'consolas', 30, False,
						   YELLOW, screen, ws[0] / 2, ws[1] / 12 * 5)
		...
		frwk.draw_text('Borderless Mode', 'consolas', 30, False,
					   BLACK, screen, ws[0] / 2, ws[1] / 60 * 31)
		if button_2.collidepoint((mx, my)):
			frwk.draw_rect(button_2, screen, MEDIUM_GREY, ws[0] / 2, ws[1] / 2)
			frwk.draw_text('Borderless Mode', 'consolas', 30,
						   False, YELLOW, screen, ws[0] / 2, ws[1] / 60 * 31)
			if click:
				frwk.fade_out(frwk.get_win_size(screen), screen)
				game(1)
				alpha = 255
		...
		frwk.draw_text('Settings', 'consolas', 30, False, BLACK,
					   screen, ws[0] / 2, ws[1] / 60 * 37)
		if button_3.collidepoint((mx, my)):  # Khi dí trỏ chuột vào
			frwk.draw_rect(button_3, screen, MEDIUM_GREY,
						   ws[0] / 2, ws[1] / 5 * 3)
			frwk.draw_text('Settings', 'consolas', 30, False,
						   YELLOW, screen, ws[0] / 2, ws[1] / 60 * 37)
			if click:  # Khi bấm chuột vào
				frwk.fade_out(frwk.get_win_size(screen), screen)
				ws, screen = frwk.show_settings(music_on, fullscreen, res_w, res_h, frwk.get_win_size(
					screen), monitor_size, screen, ws, mainClock, framerate)
				alpha = 255

		button_4 = pygame.Rect(165, 300, 270, 50)
		frwk.draw_rect(button_4, screen, GREY, ws[0] / 2, ws[1] / 10 * 7)
		frwk.draw_text('Quit', 'consolas', 30, False, BLACK,
					   screen, ws[0] / 2, ws[1] / 60 * 43)
		if button_4.collidepoint((mx, my)):  # Khi dí trỏ chuột vào
			frwk.draw_rect(button_4, screen, MEDIUM_GREY,
						   ws[0] / 2, ws[1] / 10 * 7)
			frwk.draw_text('Quit', 'consolas', 30, False, YELLOW,
						   screen, ws[0] / 2, ws[1] / 60 * 43)
			if click:  # Khi bấm chuột vào
				pygame.quit()
				sys.exit()

		frwk.draw_text('© by Constance, v1.0', 'consolas', 15,
					   False, BLACK, screen, ws[0] / 2, ws[1] - 15)
...
def game(choice):
	...
	while running:
		...
		for event in pygame.event.get():
			...
			if event.type == KEYDOWN:
				if event.key == K_ESCAPE:
					exit_to_menu, ws, screen = frwk.pause_menu(
						exit_to_menu, fullscreen, frwk.get_win_size(screen), screen, ws, mainClock, framerate)
		...
		if running:
			...
			if score >= 30:
				frwk.particles_generate(particles, screen, BLACK, snake_body[0][0] + frwk.head_img.get_width(
				) / 2, snake_body[0][1] + frwk.head_img.get_height() / 2)
			...
			screen.blit(head_img_copy, pygame.Rect(
				snake_body[0][0], snake_body[0][1], m, m))  # Đầu
			screen.blit(frwk.food_img, pygame.Rect(
				food_pos[0], food_pos[1], m, m))  # Thức ăn

			# Vẽ boost
			if boost_flag:
				if index == 0:  # Score boost
					screen.blit(frwk.boost_img[0], pygame.Rect(
						boost_pos[0], boost_pos[1], m, m))
				else:  # Speed boost
					screen.blit(frwk.boost_img[1], pygame.Rect(
						boost_pos[0], boost_pos[1], m, m))

			# Vẽ thanh hiệu lực của boost
			if not boost_flag and boost_time > 0:
				if index == 0:
					frwk.draw_text('x5 Score ', 'consolas', 15,
								   False, RED, screen, ws[0] / 20 * 7, 20)
					duration_bar = pygame.Rect(
						ws[0] / 12 * 5, 18, round(boost_time * 1.5), 20)
				else:
					frwk.draw_text('x1.5 Speed ', 'consolas', 15,
								   False, RED, screen, ws[0] / 20 * 7, 20)
					duration_bar = pygame.Rect(
						ws[0] / 12 * 5, 18, boost_time, 20)
			...
			if choice == 0:
				...
				pygame.draw.rect(
					screen, BLACK, (10, 10, ws[0] - 20, ws[1] - 20), 2)
				# X axis
				if snake_pos[0] > ws[0] - 30 or snake_pos[0] < 20:
					running, exit_to_menu, ws, screen = frwk.game_over(
						exit_to_menu, running, fullscreen, screen, ws, mainClock, framerate, score, high_score)
					if running:
						snake_pos = [100, 60]
						snake_body = [[100, 60], [80, 60], [60, 60]]
						score = 0
